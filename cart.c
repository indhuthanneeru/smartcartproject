...
struct product
{
    ...
    char id[20];// RFID tag associated with product
    int stock;// Available quantity in inventory
...
struct bank
{
    ...
    char atm_pin[10];// ATM PIN for authentication
};
			...
			return;
                ...
                else
                {
                    ...
                    if(chances == 0)
                    {
                        ...
                        uart_tx('$');
			printf("Card Blocked\n");
                        printf("switching to cash payment\n");
			sleep(1);
                       // transaction_cash();
                        return;
                    }
